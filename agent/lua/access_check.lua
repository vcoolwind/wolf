...
local function check_url_permission(appID, action, resName, clientIP)
    ...
    for i = 1, retry_max do
        ...
        --ngx.log(ngx.INFO, "start check permission request:", url)
        res = ngx.location.capture(url)
        --ngx.log(ngx.INFO, "end check permission request:", url ,", status:", res.status, ",body:", tostring(res.body))
        if res then
...
local function access_check()
    ...
    -- 设置token进header
    ngx.req.set_header("x-rbac-token", tostring(token))

    local userID = -1
    local username = nil
    local nickname = nil
    if type(userInfo) == 'table' then
        ngx.log(ngx.INFO, " add headers  userInfo:", tostring(json.dumps(userInfo)),",url is ",url)
        ngx.req.set_header("X-UserId", userInfo.id)
        ngx.req.set_header("X-Username", userInfo.username)
        ngx.req.set_header("X-UserEmail", userInfo.email)
    ...
    if ok then
        ---
        --ngx.log(ngx.INFO, "ok ====================> ok is ", ok)
    else
        ngx.log(ngx.INFO, " not ok ====================> ok is ", ok)
        -- no permission.
