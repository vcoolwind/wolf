<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.action" class="filter-item" :placeholder="this.$t('m.httpMethod')" size="small" clearable style="width: 140px;">
        <el-option v-for="action in actions" :key="action" :label="action" :value="action" />
      </el-select>
      <!-- <div class="filter-item">Request Status:</div> -->
      <el-select v-model="listQuery.status" class="filter-item" :placeholder="this.$t('m.requestStatus')" size="small" clearable style="width: 150px;">
        <el-option v-for="status in statuses" :key="status.status" :label="$t(status.description)" :value="status.status" />
      </el-select>
      <!-- <div class="filter-item">Url:</div> -->
      <el-input
        v-model="listQuery.resName"
        :placeholder="$t('m.urlForFullMatch')"
        style="width: 200px;"
        class="filter-item"
        maxlength="128"
        clearable
      ...
      <el-input
        ...
        :placeholder="$t('m.usernameOrNickname')"
        style="width: 200px;"
        class="filter-item"
        maxlength="128"
        clearable
    ...
    <el-table :data="accessLogs" style="margin-top:30px; " border>
      ...
      <el-table-column align="center" :label="$t('m.username')" min-width="25" prop="username" show-overflow-tooltip :formatter="usernameFormat" />
      <el-table-column align="center" :label="$t('m.method')" min-width="10" show-overflow-tooltip>
        <template slot-scope="scope">
          {{ scope.row.action }}
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" :label="$t('m.status')" min-width="10" show-overflow-tooltip>
        <template slot-scope="scope">
          {{ scope.row.status }}
        </template>
      </el-table-column>
      <el-table-column align="center" :label="$t('m.accessTime')" min-width="20" show-overflow-tooltip prop="accessTime" :formatter="unixtimeFormat" />
      <el-table-column align="center" :label="$t('m.clientIp')" min-width="15" show-overflow-tooltip prop="ip" />
      <el-table-column align="center" :label="$t('m.operations')" min-width="10" show-overflow-tooltip>
        <template slot-scope="scope">
  ...
  data() {
    return {
      ...
      statuses: [
        ...
        { status: 200, description: 'm.httpOk' },
        { status: 401, description: 'm.unauthorized' },
        { status: 403, description: 'm.forbidden' },
        { status: 400, description: 'm.badRequest' },
      ],
    }
  },
  computed: {
